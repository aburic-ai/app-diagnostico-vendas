...
// Estado global simulado do evento
...
export function Admin() {
  ...
  useEffect(() => {
    if (eventState.isLive && !eventState.isPaused) {
  ...
  }, [eventState.isLive, eventState.isPaused])
  ...
  const handleToggleLive = () => {
    setEventState(prev => ({
      ...
      isLive: !prev.isLive,
      isPaused: false,
      participantsOnline: prev.isLive ? 0 : Math.floor(Math.random() * 100) + 50,
    }))
  }

  const handleTogglePause = () => {
    setEventState(prev => ({ ...prev, isPaused: !prev.isPaused }))
  }

  const handlePrevModule = () => {
    if (eventState.currentModule > 0) {
      setEventState(prev => ({ ...prev, currentModule: prev.currentModule - 1 }))
    }
  }
  ...
  const handleSelectModule = (moduleId: number) => {
    setEventState(prev => ({ ...prev, currentModule: moduleId }))
  }
        ...
        >
          ...
          <div
            style={{
              flex: 1,
              padding: '16px',
              background: eventState.isLive
                ? 'rgba(239, 68, 68, 0.1)'
                : 'rgba(100, 116, 139, 0.1)',
              border: `1px solid ${eventState.isLive ? 'rgba(239, 68, 68, 0.4)' : 'rgba(100, 116, 139, 0.3)'}`,
              borderRadius: '12px',
              display: 'flex',
              alignItems: 'center',
              gap: '12px',
          ...
          >
            <div
              style={{
                ...
                height: '12px',
                borderRadius: '50%',
                background: eventState.isLive ? '#EF4444' : '#64748B',
                boxShadow: eventState.isLive ? '0 0 12px #EF4444' : 'none',
                animation: eventState.isLive ? 'pulse 1.5s infinite' : 'none',
              }}
            />
            <div>
              ...
              <p
                style={{
                  fontSize: '16px',
                  fontWeight: 'bold',
                  color: eventState.isLive ? '#EF4444' : theme.colors.text.muted,
                  margin: 0,
                }}
              >
                {eventState.isLive ? (eventState.isPaused ? 'PAUSADO' : 'AO VIVO') : 'OFFLINE'}
              </p>
            </div>
          </div>
          ...
          >
            ...
            <div>
              <p style={{ fontSize: '12px', color: theme.colors.text.muted, margin: 0 }}>ONLINE</p>
              <p
                style={{
                  fontSize: '16px',
                  fontWeight: 'bold',
        ...
        >
          ...
          <div style={{ display: 'flex', gap: '12px', marginBottom: '20px' }}>
            <motion.button
              ...
              style={{
                ...
                background: eventState.isLive
                  ? 'rgba(239, 68, 68, 0.2)'
                  : 'rgba(34, 211, 238, 0.2)',
                border: `1px solid ${eventState.isLive ? 'rgba(239, 68, 68, 0.5)' : 'rgba(34, 211, 238, 0.5)'}`,
                borderRadius: '10px',
                cursor: 'pointer',
                display: 'flex',
                alignItems: 'center',
                ...
                gap: '8px',
                color: eventState.isLive ? '#EF4444' : theme.colors.accent.cyan.DEFAULT,
                fontSize: '14px',
                fontWeight: 'bold',
              }}
            >
              <Power size={18} />
              {eventState.isLive ? 'ENCERRAR' : 'INICIAR TRANSMISSÃO'}
            </motion.button>
            ...
            {eventState.isLive && (
              ...
              >
                {eventState.isPaused ? <Play size={18} /> : <Pause size={18} />}
                {eventState.isPaused ? 'RETOMAR' : 'PAUSAR'}
              </motion.button>
            )}
          </div>
              ...
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '8px' }}>
                {eventState.isLive ? (
                  ...
                  >
                    ...
                    />
                    <span style={{ fontSize: '9px', fontWeight: 'bold', color: '#FF4444' }}>
                      {eventState.isPaused ? 'PAUSADO' : 'AO VIVO'}
                    </span>
                  </div>
                ) : (
            ...
            {eventState.isLive && !eventState.isPaused && (
              ...
              >
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                  ...
                  <span style={{ fontSize: '9px', color: theme.colors.text.muted }}>9:42</span>
                </div>
                <div
                  style={{
                    marginTop: '8px',

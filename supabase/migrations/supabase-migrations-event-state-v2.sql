-- Migration: Event State - Estado Global do Evento (v2 - Idempotente)
-- Data: 2026-02-02
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_event_state_singleton ON public.event_state ((1));
...
-- Drop trigger se existir, depois recriar
DROP TRIGGER IF EXISTS trigger_event_state_updated_at ON public.event_state;
...
DROP POLICY IF EXISTS "Anyone can read event state" ON public.event_state;
DROP POLICY IF EXISTS "Only admins can update event state" ON public.event_state;
DROP POLICY IF EXISTS "Only admins can insert event state" ON public.event_state;
...
DO $$
BEGIN
  -- Tentar adicionar a tabela ao Realtime (ignora se já existir)
...
-- Verificação final
SELECT 'Event State Migration completed successfully!' as status;
